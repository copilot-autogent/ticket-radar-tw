...
function catalogMarkup(catalog: { opentix: CatalogState | null; udn: CatalogState | null }, watchedIds: string[]): string {
  ...
  return `<section aria-labelledby="catalog-heading"><h2 id="catalog-heading">Event catalog</h2>
    <p>Discovery is informational; watched availability remains separate. OPENTIX: ${catalog.opentix?.completeness.eventCount ?? 0} summaries (${catalog.opentix?.coverage?.detailEnriched ?? 0} detail-enriched, ${catalog.opentix?.coverage?.pricesKnown ?? 0}/${catalog.opentix?.coverage?.performancesDiscovered ?? 0} prices, ${catalog.opentix?.coverage?.categoriesKnown ?? 0}/${catalog.opentix?.coverage?.summariesDiscovered ?? 0} categories; ${catalog.opentix?.completeness.status ?? "stale"}); UDN: ${catalog.udn?.completeness.eventCount ?? 0} summaries (${catalog.udn?.coverage?.detailEnriched ?? 0} detail-enriched, ${catalog.udn?.coverage?.pricesKnown ?? 0}/${catalog.udn?.coverage?.performancesDiscovered ?? 0} prices, ${catalog.udn?.coverage?.categoriesKnown ?? 0}/${catalog.udn?.coverage?.summariesDiscovered ?? 0} categories; ${catalog.udn?.completeness.status ?? "stale"}). Watch requests require GitHub sign-in plus maintainer manual validation and commit; clicking does not add a watch.</p>
