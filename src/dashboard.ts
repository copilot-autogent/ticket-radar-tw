...
import { cheapestUdnExactPositive, selectUdnTiers } from "./catalog.js";
...
function catalogMarkup(catalog: { opentix: CatalogState | null; udn: CatalogState | null }, watchedIds: string[]): string {
  ...
  for (const watchedId of watchedIds) {
    ...
    events.push({ schemaVersion: 1, source: source as "opentix" | "udn", eventId, sourceUrl: source === "opentix" ? `https://www.opentix.life/event/${eventId}` : `https://tickets.udnfunlife.com/Application/UTK02/UTK0201_.aspx?PRODUCT_ID=${eventId}`, title: "Watched event (catalog not currently discovered)", category: "unknown", classificationReason: "unknown", classificationConfidence: "low", firstSeenAt: new Date(0).toISOString(), catalogFetchedAt: new Date(0).toISOString(), detailStatus: "discovered-summary", provenance: { source: source as "opentix" | "udn", sourceUrl: source === "opentix" ? `https://www.opentix.life/event/${eventId}` : `https://tickets.udnfunlife.com/Application/UTK02/UTK0201_.aspx?PRODUCT_ID=${eventId}`, retrievedAt: new Date(0).toISOString(), discoveryMethod: "structured-summary", parserVersion: "placeholder" }, performances: [] });
  }
  ...
  return `<section aria-labelledby="catalog-heading"><h2 id="catalog-heading">Event catalog</h2>
    <p>Discovery is informational; watched availability remains separate. ${["opentix", "udn"].map((source) => { const state = catalog[source as "opentix" | "udn"]; const summaries = state?.coverage?.summariesDiscovered ?? state?.completeness.eventCount ?? 0; const performances = state?.coverage?.performancesDiscovered ?? 0; const failures = state?.completeness.detailFailureCount ?? 0; return `${source.toUpperCase()}: ${summaries} summaries; cap/scan: ${state?.completeness.status ?? "stale"} (lower-bound unless an authoritative provider total is recorded); detail-enriched/discovered: ${state?.coverage?.detailEnriched ?? 0}/${summaries}; known minimum advertised prices/enriched performances: ${state?.coverage?.pricesKnown ?? 0}/${performances}; source-evidenced categories/discovered summaries: ${state?.coverage?.categoriesKnown ?? 0}/${summaries}${failures ? `; ${failures} detail request(s) failed, retained facts are stale` : ""}`; }).join(". ")}. Watch requests require GitHub sign-in plus maintainer manual validation and commit; clicking does not add a watch.</p>
      ...
      (() => {
        ...
        function render() {
          ...
          const rows = data.events.filter((event) => {
            ...
            if (state.view === "upcoming" && dates.length > 0 && !dates.some((d) => d >= now)) return false;
          ...
          $("catalog-results").innerHTML = rows.length ? "<table><caption>Catalog results</caption><thead><tr><th>Platform</th><th>Event</th><th>Category</th><th>Performances</th><th>Dates / prices</th><th>Watch</th></tr></thead><tbody>" + rows.map((e) => {
            ...
            return "<tr><td>" + esc(e.source) + "</td><td><a href='" + esc(e.sourceUrl) + "'>" + esc(e.title) + "</a></td><td>" + esc(e.category) + "</td><td>" + e.performances.length + " · " + esc(e.detailStatus) + "</td><td>" + (e.performances.length ? e.performances.map((p) => esc((p.startsAt || "date unknown") + " · " + (p.minPrice == null ? "price unknown" : p.maxPrice == null ? p.minPrice + "–unknown TWD" : p.minPrice + "–" + p.maxPrice + " TWD"))).join("<br>") : "Details incomplete") + "</td><td><a href='" + esc(request) + "'>Request watch</a></td></tr>";
...
export async function renderLiveDashboard(root: string, event: NormalizedEvent | null, state: RuntimeState, udn: UdnRuntimeState | null = null, ticketPlus: TicketPlusRuntimeState | null = null, catalog: { opentix: CatalogState | null; udn: CatalogState | null } = { opentix: null, udn: null }): Promise<void> {
  ...
  const rows = performances.map((item) => `<tr><td>${display(item.startsAt)}</td><td>${escapeHtml(item.lifecycle)}</td><td>${display(item.remaining)}</td><td>${display(item.price.min)}–${display(item.price.max)} TWD (advertised)</td></tr>`).join("");
  const udnTiers = udn?.snapshot?.tiers ?? [];
  const udnAvailableTiers = selectUdnTiers(udnTiers);
  const cheapestUdn = cheapestUdnExactPositive(udnTiers);
  const renderUdnTier = (tier: typeof udnTiers[number]) => `<tr><td>${escapeHtml(tier.label)}</td><td>${display(tier.priceTwd)} TWD</td><td>${escapeHtml(tier.availability)}</td><td>${display(tier.exactCount)}</td><td><a href="${escapeHtml(tier.sourceUrl)}">Official purchase</a></td></tr>`;
  const udnRows = udnAvailableTiers.map(renderUdnTier).join("");
  const udnUnavailableRows = udnTiers.filter((tier) => !selectUdnTiers([tier]).length).map(renderUdnTier).join("");
  ...
  await writeJsonAtomic(resolve(root, "public/data/dashboard.json"), { source: state.source, parserVersion: state.snapshot?.parserVersion ?? null, event: event ?? state.snapshot, health: state.health, history: state.history, lastAttemptAt: state.lastAttemptAt, lastSuccessfulAt: state.lastSuccessfulAt, retainedDataAt: state.retainedDataAt, catalog, transitions: state.transitions.slice(-20), sources: { opentix: { health: state.health, event: event ?? state.snapshot }, udn: udn ?? { health: { category: "not-run" }, snapshot: null }, ticketPlus: ticketPlus ?? { health: { category: "not-run" }, ordinary: null, lottery: null, pairings: [], conflicts: [] } } });
}
