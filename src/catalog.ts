...
export type CatalogDetailStatus = "discovered-summary" | "detail-enriched" | "detail-failed";
...
export interface CatalogCompleteness {
  ...
  health: "ok" | "stale" | "error" | "not-run"; error?: string; detailFailureCount?: number; detailFailureReasons?: string[];
...
export function mergeCatalogState(previous: CatalogState | null, incoming: CatalogState): CatalogState {
  ...
  const events = keys.map((key) => { const old = oldByKey.get(key); const next = incomingByKey.get(key); if (!next) return old!; const merged = mergeSummary(old, next); return { ...merged, performances: next.performances.length ? next.performances : old?.performances ?? [], detailStatus: next.performances.length ? "detail-enriched" : old?.detailStatus ?? next.detailStatus ?? "discovered-summary" } as CatalogEvent; }); const stale = normalizedIncoming.completeness.health !== "ok"; const completeness = { ...normalizedIncoming.completeness, eventCount: events.length, status: stale ? "stale" as const : normalizedIncoming.completeness.status }; return { ...normalizedIncoming, events, summaries: events.map(({ performances: _performances, ...summary }) => summary), completeness, coverage: coverage(events) };
}
export async function discoverCatalog(source: CatalogSource, options: { now?: Date; fetchImpl?: typeof fetch; indexUrls?: string[]; watchedIds?: string[]; interestTerms?: string[]; } = {}): Promise<CatalogState> {
  ...
  const allowedHost = source === "opentix" ? "www.opentix.life" : "tickets.udnfunlife.com";
  const readResponse = async (response: Response): Promise<string> => {
    if (response.url && new URL(response.url).hostname !== allowedHost) throw new Error("redirect-outside-allowlist");
    const contentType = response.headers.get("content-type") ?? "";
    if (contentType && !/(?:text\/html|application\/xhtml\+xml|application\/json|text\/plain)/i.test(contentType)) throw new Error("unexpected-content-type");
    const body = await response.text();
    if (body.length > MAX_CATALOG_RESPONSE_BYTES) throw new Error("response-size");
    return body;
  };
  try { for (const index of configuredUrls.slice(0, CATALOG_PAGE_CAP)) { const response = await fetchImpl(index, { headers: { accept: "text/html,application/xhtml+xml,application/ld+json" }, redirect: "follow" }); pageCount++; if (!response.ok) { pageError = `http-${response.status}`; break; } for (const summary of parseCatalogSummaries(await readResponse(response), source, now)) { if (summaries.size < CATALOG_EVENT_CAP || summaries.has(`${source}:${summary.eventId}`)) summaries.set(`${source}:${summary.eventId}`, summary); } if (summaries.size >= CATALOG_EVENT_CAP) break; } } catch (error) { pageError = error instanceof Error ? error.message : "discovery-failure"; }
  ...
  for (const summary of prioritized) { detailCount++; try { const response = await fetchImpl(summary.sourceUrl, { headers: { accept: "text/html,application/xhtml+xml,application/ld+json" }, redirect: "follow" }); if (!response.ok) throw new Error(`http-${response.status}`); const html = await readResponse(response); let title = summary.title; let performances: CatalogPerformance[] = []; if (source === "opentix") { const parsed = parseOpentixHtml(html, timestamp, now); title = parsed.title; performances = parsed.performances.map((item) => performanceFromOpentix(item, summary.eventId, summary.sourceUrl)); } else { const parsed = parseUdnCatalogDetailHtml(html, summary.sourceUrl, now); title = parsed.title || title; performances = parsed.performances; } const old = byKey.get(`${source}:${summary.eventId}`)!; byKey.set(`${source}:${summary.eventId}`, { ...old, title, detailStatus: "detail-enriched", ...(htmlArtist(html) ? { artist: htmlArtist(html) } : {}), ...classifyEvent(title, old.sourceCategory), catalogFetchedAt: timestamp, provenance: { ...old.provenance, retrievedAt: timestamp, discoveryMethod: "detail", parserVersion: sourceParserVersion }, performances }); } catch (error) { const reason = error instanceof Error ? error.message : "detail-failure"; failureReasons.push(reason); const old = byKey.get(`${source}:${summary.eventId}`)!; byKey.set(`${source}:${summary.eventId}`, { ...old, detailStatus: "detail-failed" }); } }
  const events = [...byKey.values()] as CatalogEvent[]; const truncated = pageCount >= CATALOG_PAGE_CAP || summaries.size >= CATALOG_EVENT_CAP; const status: CatalogCompletenessStatus = pageError || failureReasons.length ? "stale" : "truncated"; const completeness: CatalogCompleteness = { source, fetchedAt: timestamp, eventCount: events.length, pageCount, detailCount, stopReason: pageError ? "partial" : truncated ? "cap" : "normal-exhaustion", status, health: pageError && events.length === 0 ? "error" : pageError || failureReasons.length ? "stale" : "ok", ...(pageError ? { error: pageError } : {}), ...(failureReasons.length ? { detailFailureCount: failureReasons.length, detailFailureReasons: [...new Set(failureReasons)].slice(0, 10) } : {}) }; return { schemaVersion: CATALOG_SCHEMA_VERSION, generatedAt: timestamp, summaries: events.map(({ performances: _performances, ...summary }) => summary), events, completeness, coverage: coverage(events) };
