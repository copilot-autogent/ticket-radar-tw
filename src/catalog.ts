...
export function mergeCatalogState(previous: CatalogState | null, incoming: CatalogState): CatalogState {
  if (!previous) return incoming; const oldByKey = new Map(previous.events.map((event) => [`${event.source}:${event.eventId}`, event])); const incomingByKey = new Map(incoming.events.map((event) => [`${event.source}:${event.eventId}`, event])); const keys = [...new Set([...oldByKey.keys(), ...incomingByKey.keys()])].slice(0, CATALOG_EVENT_CAP);
  const events = keys.map((key) => { const old = oldByKey.get(key); const next = incomingByKey.get(key); if (!next) return old!; const merged = mergeSummary(old, next); return { ...merged, performances: next.performances.length ? next.performances : old?.performances ?? [], detailStatus: next.performances.length ? "detail-enriched" : old?.detailStatus ?? "discovered-summary" } as CatalogEvent; }); const stale = incoming.completeness.health !== "ok"; const completeness = { ...incoming.completeness, eventCount: events.length, status: stale ? "stale" as const : incoming.completeness.status }; return { ...incoming, events, summaries: events.map(({ performances: _performances, ...summary }) => summary), completeness, coverage: coverage(events) };
}
export async function discoverCatalog(source: CatalogSource, options: { now?: Date; fetchImpl?: typeof fetch; indexUrls?: string[]; watchedIds?: string[]; interestTerms?: string[]; } = {}): Promise<CatalogState> {
  const fetchImpl = options.fetchImpl ?? fetch; const now = options.now ?? new Date(); const timestamp = now.toISOString(); const configuredUrls = options.indexUrls ?? [source === "opentix" ? "https://www.opentix.life/" : "https://tickets.udnfunlife.com/"]; const summaries = new Map<string, CatalogSummary>(); let pageCount = 0; let detailCount = 0; let pageError: string | undefined;
  try { for (const index of configuredUrls.slice(0, CATALOG_PAGE_CAP)) { const response = await fetchImpl(index, { headers: { accept: "text/html,application/xhtml+xml,application/ld+json" }, redirect: "follow" }); pageCount++; if (!response.ok) { pageError = `http-${response.status}`; break; } for (const summary of parseCatalogSummaries(await response.text(), source, now).slice(0, CATALOG_PAGE_SIZE)) { if (summaries.size < CATALOG_EVENT_CAP || summaries.has(`${source}:${summary.eventId}`)) summaries.set(`${source}:${summary.eventId}`, summary); } if (summaries.size >= CATALOG_EVENT_CAP) break; } } catch (error) { pageError = error instanceof Error ? error.message : "discovery-failure"; }
